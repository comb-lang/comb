...
DEFAULT_ARENA_STATIC_RESERVE_SIZE ::
    mem.Gigabyte when size_of(uintptr) == 8 else 128 * mem.Megabyte


Allocator_Error :: mem.Allocator_Error

@(require_results, no_sanitize_address)
reserve :: proc "contextless" (
    size: uint,
    address_hint := uintptr(0),
) -> (
    data: []byte,
    err: Allocator_Error,
) {
...
memory_block_alloc :: proc(
    committed, reserved: uint,
    alignment: uint = 0,
    flags: Memory_Block_Flags = {},
) -> (
    block: ^Memory_Block,
    err: Allocator_Error,
) {
    ...
    base_offset := mem.align_forward_uintptr(
        size_of(Platform_Memory_Block),
        max(uintptr(alignment), align_of(Platform_Memory_Block)),
    )
...
@(private = "file", require_results, no_sanitize_address)
memory_block_ensure_capacity :: proc(
    block: ^Memory_Block,
    capacity: uint,
    ...
    if block.committed < capacity {
        ...
        platform_total_commit = min(
            max(platform_total_commit, default_commit_size),
            pmblock.reserved,
        )
...
@(require_results, no_sanitize_address)
memory_block_resize :: proc(
    block: ^Memory_Block,
    new_size: uint,
    default_commit_size: uint = 0,
) -> (
    err: Allocator_Error,
) {
...
@(require_results, no_sanitize_address)
alloc_from_memory_block :: proc(
    block: ^Memory_Block,
    min_size, alignment: uint,
    default_commit_size: uint = 0,
) -> (
    data: []byte,
    err: Allocator_Error,
    ...
    @(no_sanitize_address)
    calc_alignment_offset :: proc "contextless" (
        block: ^Memory_Block,
        alignment: uintptr,
    ) -> uint {
